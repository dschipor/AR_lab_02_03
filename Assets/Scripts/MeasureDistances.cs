...
public class MeasureDistances : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        for (int i = 0; i < distances.Count; i++)
        {
            ...
            distances[i].transform.position = Camera.main.WorldToScreenPoint(new Vector3(lines[i].transform.position.x,
                    lines[i].transform.position.y, lines[i].transform.position.z));
        }
        ...
        if (m_RaycastManager.Raycast(touchPosition, s_Hits, TrackableType.PlaneWithinPolygon))
        {
            ...
            if (points.Count > 1)
            {
                ...
                line.transform.position = (points[points.Count - 1].transform.position + points[points.Count - 2].transform.position) / 2;
                ...
                float cubeSizeInverse = 20;
                float scaleFactor = distance * cubeSizeInverse;
                line.transform.localScale = new Vector3(0.5f, 0.5f, line.transform.localScale.z * scaleFactor); /* !0.5 can be changed to whatever value we want
                ...
                distanceSum = 0;
                for (int i = 0; i < distances.Count; i++)
                    distanceSum += float.Parse(distances[i].text);
